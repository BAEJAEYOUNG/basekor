...
CoreForm.prototype.createForm = function(opt) {
    if(this.form == null || typeof(this.form) == 'undefined') {
        this.form = $('<form></form>');
    }
    this.form.empty();
    if(typeof(opt.id) != 'undefined') {
        this.form.attr('id', opt.id);
    }
    if(typeof(opt.name) != 'undefined') {
        this.form.attr('name', opt.name);
    }
    this.form.attr('method', opt.method || 'post');
    this.form.attr('action', opt.action || '');
    this.form.attr('target', opt.target || '_self');
    return this;
};
CoreForm.prototype.attachHiddenElement = function(name, value) {
    if(!this.form) {
        $.Utils.alert('createForm must be called.');
        return;
    }
